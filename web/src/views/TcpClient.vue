  ...
  <div class="tcp-client">
    <el-card class="main-card">
      ...
      <el-table :data="channels" border stripe>
        ...
        <el-table-column prop="tcp_client.host" label="主机地址" width="150">
          <template #default="{ row }">
            {{ row.tcp_client?.host || '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="tcp_client.port" label="端口" width="100">
          <template #default="{ row }">
            {{ row.tcp_client?.port || '-' }}
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="showAddDialog" :title="isEdit ? '编辑通道' : '添加TCP客户端通道'" width="600px">
      <el-form :model="channelForm" label-width="120px">
        ...
        <el-form-item label="主机地址">
          <el-input v-model="channelForm.tcp_client.host" placeholder="例如: 192.168.1.100" />
        </el-form-item>
        <el-form-item label="端口">
          <el-input-number v-model="channelForm.tcp_client.port" :min="1" :max="65535" />
        </el-form-item>
...
const loadChannels = async () => {
  try {
    const response = await api.get('/channels/tcp-client')
...
const saveChannel = async () => {
  try {
    ...
    if (isEdit.value) {
      await api.put(`/channels/tcp-client/${channelForm.id}`, data)
    } else {
      await api.post('/channels/tcp-client', data)
    }
...
const editChannel = (row) => {
  ...
  Object.assign(channelForm, {
    ...
    tcp_client: {
      host: row.tcp_client?.host || '',
      port: row.tcp_client?.port || 8080
    }
  })
...
const deleteChannel = async (row) => {
  ElMessageBox.confirm('确定删除此通道？', '提示', {
    ...
    try {
      await api.delete(`/channels/tcp-client/${row.id}`)
      await loadChannels()
